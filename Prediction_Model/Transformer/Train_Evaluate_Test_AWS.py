...
output_size = 2
num_heads = 12 # should be a factor of input_size
num_layers = 10
num_epochs = 100
learning_rate = 0.001
weight_decay = 0.01 # for Adam optimizer
...
for epoch in range(num_epochs):
    ...
    for i, (batch_X, batch_y, batch_y_binary) in loop:
        ...
        loss_classification = criterion_classification(outputs_binary, batch_y_binary)
        loss = loss_regression + loss_classification
        train_accuracy += (outputs_binary.round() == batch_y_binary).float().mean()
        loss.backward()
        optimizer.step()
    ...
    # Validate every 1 epochs and save the best model
    if (epoch + 1) % 1 == 0:
        model.eval()
        val_loss = 0
        val_accuracy = 0
        ...
        with torch.no_grad():
            for i, (batch_X, batch_y_continuous, batch_y_binary) in val_loop:
                ...
                val_accuracy += (outputs_binary.round() == batch_y_binary).float().mean()
