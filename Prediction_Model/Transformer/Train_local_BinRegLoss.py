...
for epoch in range(num_epochs):
    ...
    for i, (batch_X, batch_y, batch_y_binary) in loop:
        ...
        loss_regression = criterion_regression(outputs, batch_y) # unreduced
        correct_prediction = (outputs_binary.round() == batch_y_binary).float()
        loss = ((1 - correct_prediction) * loss_regression).mean()
        train_accuracy += correct_prediction.mean()
        loss.backward()
        optimizer.step()
    ...
    ave_accuracy = train_accuracy / len(train_loader)
    avg_loss = total_loss / len(train_loader)
    print(f'Epoch: {epoch + 1}/{num_epochs}, Average Loss: {avg_loss}, Average Accuracy: {ave_accuracy}')
    ...
    if (epoch + 1) % 5 == 0:
        ...
        with torch.no_grad():
            for i, (batch_X, batch_y_continuous, batch_y_binary) in val_loop:
                ...
                correct_prediction = (outputs_binary.round() == batch_y_binary).float()
                loss = ((1 - correct_prediction) * loss_regression).mean()
                val_accuracy += correct_prediction.mean()
                val_loss += loss.item()
        ...
        if val_loss < best_val_loss:
            ...
            print(f'Saving the new best model at epoch {epoch+1} with validation loss {best_val_loss} and validation accuracy {val_accuracy}')
            ...
            print(f'Validation loss did not improve from {best_val_loss} with corresponding validation accuracy of: {best_val_accuracy}')
...
with torch.no_grad():
    ...
    for i, (batch_X, batch_y_continuous, batch_y_binary) in enumerate(test_loader):
        ...
        outputs, outputs_binary = model(batch_X.float())
        correct_prediction = (outputs_binary.round() == batch_y_binary).float()
        test_accuracy += correct_prediction.mean()
...
ave_test_accuracy = test_accuracy / len(test_loader)
