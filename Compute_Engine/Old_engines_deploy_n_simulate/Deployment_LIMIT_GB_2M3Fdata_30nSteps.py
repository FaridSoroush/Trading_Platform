...
# print("time to match UNIT time: ", initial_wait_time)
...
position_ratio=0.0
quantity= 0.045 # 4.5% of $100 account is equivalent to 
                # risking 0.00015 (1.5e-4) BTC (at $30K
                # price) per trade
type="MARKET"
...
def trade_evaluation():
    ...
    quantity_long=position_ratio*USDT_balance/current_price
    quantity_short=position_ratio*BTC_balance
    ...
    # if you turn this OFF, it's gonna TRADE NO MATTER WHAT (the next block) 
    # until quantitiy of the asset reaches 0 and you'll get API error.
    # When it's ON, it's gonna check that the quantity of the (potentially 
    # profitable) trade is greater than the minimum trade size (step_size)
    # else:
    ...
    if quantity != 0 and quantity < 1e-4 and (side == 'BUY' or side == 'SELL'):  
        ...
        if BTC_balance > 1e-4 and side=='SELL': # if the balance is greater than the minimum trade size
            ...
            side = 'NONE'
            trade_approved=False
            print("not enough balance for minimum trade size")
